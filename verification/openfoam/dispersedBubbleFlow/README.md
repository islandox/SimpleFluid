...
Both drivers also support MPI on the same 40,400-cell mesh and physical schedule.
...
transport BiCGStab/SGS based on historical measurements of the scale-1 44-cell fixtures; these
settings have not been retuned for the enlarged geometry. The steady-case policy gain
...
A 10 m tall column of area 100 m² has 10 × 10 × 404 cells, a 0.1 m/s
...
saturated steam density is never used for the bubbles. The fixed absolute
gas pressure omits hydrostatic variation, approximately 98 kPa over this 10 m
water column. The enlarged case remains a prescribed transport verification,
not a realistic pressure-dependent tall bubble column.
...
time `t = 100 s` on both sides; output times are 0, 20, 40, 60, 80, and 100 s.
The timestep is unchanged; the longer endpoint allows steady escape in the
ten-times-taller column.
...
`q × column_volume = 1e-3 mol/s`. Each executable enforces the final
outlet balance to `2e-8 mol/s` and a per-step maximum concentration change
...
normalized integrated absolute error below 0.012 (the original 0.12 m
front-smearing allowance divided by column height), allowing the documented
first-order numerical diffusion. By 1 s, escaped inventory must be
between 4% and 6% of the initial inventory, bracketing the exact 5%.
Physical cell widths, velocity and transient duration are unchanged.

## Outputs and acceptance

Field output selects the central-y plane (all ten x columns); inventories and
conservation gates include all 40,400 cells. Each solver additionally writes cell bounds and temperature, liquid density,
...
Both solvers write `profiles.csv` for all 404 cells of the central x/y column at every declared
...
below `2e-10 mol` and number residual below `2e-8` relative. Solver exit
