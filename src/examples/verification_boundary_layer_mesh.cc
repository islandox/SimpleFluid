    ...
    try
    {
        if (argc != 4 && argc != 5)
            throw std::invalid_argument(
                "Usage: verification_boundary_layer_mesh CASE uniform|medium|fine OUTPUT [integer-length-scale=10]");
        const int scale = argc == 5 ? std::stoi(argv[4]) : 10;
        if (scale < 1)
            throw std::invalid_argument("Length scale must be a positive integer");
        ...
        for (int i = 0; i <= ny; ++i)
            y.push_back(depth * i / ny);
        SimpleFluid::Meshes::OrthogonalCartesian3D geometry({{x, y, z}});
        const double first = thickness * (growth - 1) / (std::pow(growth, count) - 1);
        const double first_x = (name == "bottomHeatedBubblyConvection" ? original_width / 6 : thickness) *
                               (growth - 1) / (std::pow(growth, count) - 1);
        if (refine)
        {
        ...
        stream << std::setprecision(17) << "# Native BoundaryLayerMeshFactory; " << name << " / " << level << "\n"
               << "# Length scale " << scale << "; original wall/core cell widths retained\n"
               << "# Layer count " << (refine ? count : 0) << ", first cell heights x=" << edges[0][1] - edges[0][0]
               << " z=" << edges[2][1] - edges[2][0] << ", growth " << growth << "; SI metres\n";
        size_t cells = 1;
