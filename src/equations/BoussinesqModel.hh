...
private:
    ...
    friend class TemperatureSourceRegistry;

    template<TpetraTypePack>
    friend class FissionPowerSource;

    bool requires_dynamic_update() const noexcept
    {
        return d_enabled && static_cast<bool>(d_updater);
    }
    ...
    void clear_updater_after_next_update() noexcept
    {
        d_clear_updater_after_update = static_cast<bool>(d_updater);
    }
...
public:
    ...
    void update(const context_type& context)
    {
        ...
        const auto globally_active =
            collective_detail::collective_local_validation_batch(
                *d_mesh,
                "Temperature source registry update",
                d_sources,
                [](const auto& entry)
                {
                    return entry.second->requires_dynamic_update();
                },
                [&](auto& entry)
                {
                    entry.second->update_local(context);
                },
                d_update_validation_scratch);
...
private:
    ...
    void validate_collective_schema()
    {
        ...
        {
            throw std::invalid_argument(
                "Temperature source registry has too many entries for "
        ...
        {
            throw std::invalid_argument(
                "Temperature source registry requires the same source "
                "count on every rank.");
        }
        ...
        {
            throw std::invalid_argument(
                "Temperature source registry names are too large for "
                "collective validation.");
        }
        ...
        {
            throw std::invalid_argument(
                "Temperature source registry requires identical ordered "
    ...
    collective_detail::CollectiveValidationBatchScratch
        d_update_validation_scratch;
};
