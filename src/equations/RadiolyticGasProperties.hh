...
struct RadiolyticGasOptions
{
    ...
    RadiolyticPressureMode pressure_mode =
        RadiolyticPressureMode::Constant;
    RadiolyticTransportMode dissolved_transport =
        RadiolyticTransportMode::NoAdvection;
    BubbleTransportMode bubble_transport =
        BubbleTransportMode::General;
    RadiolyticHeavisideMode heaviside_mode =
        RadiolyticHeavisideMode::Exact;
    BubbleRiseVelocityMode rise_velocity_mode =
        BubbleRiseVelocityMode::ZeroSlip;
    SurfaceTensionMode surface_tension_mode =
        SurfaceTensionMode::Constant;
    HydrogenDiffusivityMode diffusivity_mode =
...
inline real_t ideal_gas_alpha_source(
    ...
    real_t yield_mol_per_j,
    real_t power_density,
    real_t gas_constant,
    real_t temperature,
    real_t absolute_pressure,
    real_t max_source_rate)
{
    ...
    {
        throw std::invalid_argument(
            "release efficiency cannot exceed one.");
    }
    ...
    {
        throw std::invalid_argument(
            "maximum alpha source rate must be non-negative.");
    }

    const auto raw =
        liquid_fraction * release_efficiency * yield_mol_per_j
      * power_density * gas_constant * temperature / absolute_pressure;
    if (!std::isfinite(raw))
    {
        throw std::invalid_argument(
            "ideal-gas alpha source is non-finite.");
    }
    ...
    real_t atmospheric_pressure)
{
    ...
    return 5.165e-5 * std::pow(ratio, 4)
         - 1.732e-3 * std::pow(ratio, 3)
         + 0.02245 * std::pow(ratio, 2)
         - 0.1554 * ratio + 1.134;
}
    ...
    real_t uranium_concentration_mol_per_m3)
{
    ...
    return (-1.3387e-6 * temperature_kelvin - 3.4319e-5)
             * uranium_concentration_mol_per_m3
    ...
    real_t mean_let)
{
    ...
    return (-2.862e-15 * t * t + 7.3996e-13 * t - 9.9925e-11)
             * mean_let * mean_let
         + (8.7909e-14 * t * t - 9.7928e-13 * t + 3.4558e-9)
             * mean_let
    ...
    real_t hydrogen_yield_molecules_per_100_ev)
{
    ...
    require_non_negative(
        ...
        "hydrogen yield");
    if (hydrogen_yield_molecules_per_100_ev <= 0.5
        || hydrogen_yield_molecules_per_100_ev >= 4.5)
    {
        throw std::invalid_argument(
            "Winter's yield correction requires "
            "0.5 < G_H2 < 4.5 molecules per 100 eV.");
    }
    const auto yield = hydrogen_yield_molecules_per_100_ev;
    return (0.3554 + 0.4264 * yield - 0.0400 * yield * yield)
         * pure_water_radius;
}
...
inline real_t sheng2024_nucleation_radius(
    ...
    real_t atmospheric_pressure)
{
    const auto mean_let = mean_fission_fragment_let(
        temperature_kelvin, uranium_concentration_mol_per_m3);
    const auto water_radius = pure_water_nucleation_radius(
        temperature_kelvin, mean_let);
    const auto atmospheric_radius = atmospheric_nucleation_radius(
        water_radius, hydrogen_yield_molecules_per_100_ev);
    return pressure_nucleation_correction(
    ...
    real_t uranium_concentration_mol_per_m3)
{
    ...
    require_non_negative(
        uranium_concentration_mol_per_m3,
        "uranyl nitrate concentration");
    return 1.7160e-7 * temperature_celsius * temperature_celsius
         - 1.4427e-4 * temperature_celsius
         + 2.0163e-6 * uranium_concentration_mol_per_m3
         + 7.5725e-2;
}
...
inline real_t hughmark_sherwood(real_t reynolds, real_t schmidt)
{
    ...
    {
        throw std::invalid_argument(
            "Hughmark correlation requires 0 <= Sc < 250.");
    }
    if (reynolds < 776.06)
    {
        return 2.0 + 0.6 * std::sqrt(reynolds) * std::cbrt(schmidt);
    }
    return 2.0 + 0.27 * std::pow(reynolds, 0.63)
         * std::cbrt(schmidt);
}
...
inline real_t hughmark_mass_transfer_coefficient(
    ...
    real_t dynamic_viscosity,
    real_t relative_speed)
{
    ...
    const auto schmidt =
        dynamic_viscosity / (liquid_density * diffusivity);
    const auto reynolds =
        2.0 * liquid_density * relative_speed * radius
      / dynamic_viscosity;
    return hughmark_sherwood(reynolds, schmidt)
    ...
    real_t eotvos)
{
    ...
    const auto spherical =
        24.0 * (1.0 + 0.15 * std::pow(reynolds, 0.687)) / reynolds;
    const auto deformed =
        8.0 * eotvos / (3.0 * (eotvos + 4.0));
    ...
    real_t relative_tolerance = 1.0e-10)
{
    ...
    {
        throw std::invalid_argument(
            "bubble gas density must be less than liquid density.");
    }
    require_positive(dynamic_viscosity, "dynamic viscosity");
    ...
    {
        throw std::invalid_argument(
            "rise-velocity iteration limit must be positive.");
    }

    const auto density_difference = liquid_density - gas_density;
    const auto eotvos =
        4.0 * gravity * radius * radius * density_difference
      / surface_tension;
    const auto velocity_scale =
        8.0 * radius * gravity / 3.0;
    auto evaluate = [&](real_t velocity)
    {
        const auto reynolds =
            2.0 * liquid_density * velocity * radius
          / dynamic_viscosity;
        const auto drag_coefficient =
            celata2007_drag_coefficient(reynolds, eotvos);
        return std::pair{
            drag_coefficient * velocity * velocity - velocity_scale,
            drag_coefficient};
    };

    auto lower = std::numeric_limits<real_t>::min();
    auto upper = std::max(
        std::sqrt(velocity_scale), 1.0e-8);
    auto upper_evaluation = evaluate(upper);
    for (int expansion = 0;
         upper_evaluation.first < 0.0 && expansion < 64;
         ++expansion)
    {
        upper *= 2.0;
    ...
    for (int iteration = 1; iteration <= max_iterations; ++iteration)
    {
        ...
        if (std::abs(result.residual)
            <= relative_tolerance * velocity_scale)
        {
            result.reynolds =
                2.0 * liquid_density * result.velocity * radius
              / dynamic_viscosity;
            const auto diameter = 2.0 * radius;
            result.within_experimental_range =
                diameter >= 0.5e-3 && diameter <= 4.0e-3
                && result.reynolds >= 200.0
                && result.reynolds <= 1500.0
                && result.eotvos >= 0.1 && result.eotvos <= 3.5;
            return result;
        }
    ...
    real_t relative_tolerance = 1.0e-12)
{
    ...
    {
        return {
            lower_residual > 0.0 ? lower : upper,
            lower_residual > 0.0 ? lower_residual : upper_residual,
            0,
            false};
    }
    ...
    for (int iteration = 1; iteration <= max_iterations; ++iteration)
    {
        ...
        result.iterations = iteration;
        if (std::abs(result.residual)
            <= relative_tolerance * std::max(rhs, 1.0e-300))
        {
            return result;
        }
        if (result.residual < 0.0)
    ...
    real_t large_radius)
{
    const auto denominator =
        micro_number * micro_radius * micro_radius
      + large_number * large_radius * large_radius;
    if (denominator <= 0.0)
        return 0.0;
    return (micro_number * std::pow(micro_radius, 3)
          + large_number * std::pow(large_radius, 3))
         / denominator;
}
