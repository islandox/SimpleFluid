...
PlanarALEBoundary<Pack>::PlanarALEBoundary(SP<const mesh_type> mesh, std::string boundary_name, Dimension axis,
...
template<TpetraTypePack Pack>
void PlanarALEBoundary<Pack>::apply_kinematic_velocity(const FVM::ALEControlVolumeState& ale, velocity_boundary_cache_type& cache) const
{
    ale.validate(*d_mesh);
...
template<TpetraTypePack Pack>
void PlanarALEBoundary<Pack>::enforce_kinematic_flux(const FVM::ALEControlVolumeState& ale, face_flux_field_type& absolute_flux) const
{
    ale.validate(*d_mesh);
...
template<TpetraTypePack Pack>
void PlanarALEBoundary<Pack>::validate_collective_controls() const
{
...
template<TpetraTypePack Pack>
void PlanarALEBoundary<Pack>::validate(const VesselVolumeMap& volume_map)
{
    const auto communicator = d_mesh->owned_cell_map()->getComm();
...
template<TpetraTypePack Pack>
void PlanarALEBoundary<Pack>::refresh(const VesselVolumeMap& volume_map)
{
    validate(volume_map);
}
