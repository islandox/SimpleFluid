/**
 * @file MetisPartitioner.hh
 * @author islandox(59904740+islandox@users.noreply.github.com)
 * @brief ParMETIS-based mesh domain decomposition for parallel runs.
 * @version 0.1
 * @date 2026-05-29
 *
...
public:
    ...
    static bool partition(Mesh<Pack>& mesh, const Teuchos::RCP<const comm_type>& comm) {
        ...
        std::vector<char> rbuf(static_cast<std::size_t>(std::max(rd.back() + rcnt.back(), 0)));
        ...
        std::vector<char> gr_rbuf(static_cast<std::size_t>(std::max(gr_rd.back() + gr_rcnt.back(), 0)));
        ...
        // the same rank, but the ghost detection still flags them).
        {
            ...
            ghost_pkts.erase(
                std::remove_if(ghost_pkts.begin(), ghost_pkts.end(),
                    [&](const Packet& p) { return owned_gids.count(p.gid) > 0; }),
                ghost_pkts.end());
        }
...
private:
     ...
     */
    static void rebuild(Mesh<Pack>& mesh, const std::vector<Packet>& owned_pkts, const std::vector<Packet>& ghost_pkts,
